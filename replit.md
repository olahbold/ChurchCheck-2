...
- **Enhanced Member Attendance Log Reporting (August 2025)**: Completely redesigned Member Attendance Log to generate comprehensive Excel-style matrix reports showing attendance patterns across date ranges. Features include attendance summary statistics, matrix view with YES/NO status for each member per date, color-coded attendance indicators (green for present, red for absent), individual attendance percentages, sticky column headers for better navigation, and enhanced CSV export with formatted date headers and comprehensive statistics. The matrix format displays all members as rows and attendance dates as columns, providing a complete attendance overview similar to Excel spreadsheets for pastoral care and member engagement analysis.
- **External Check-in System Implementation (August 2025)**: Introduced comprehensive external check-in functionality to reduce queues at physical events. System generates unique event URLs and 6-digit PINs for secure remote check-ins. Features include admin-controlled enable/disable per event, unique URL generation with nanoid, PIN-based authentication preventing remote access, simplified check-in interface showing only church name and current time, immediate attendance record creation with 'external' method tracking, duplicate prevention for same-event same-day check-ins, and comprehensive admin management UI with URL/PIN sharing, security features display, and member instructions. External check-in page provides public access route with church branding and real-time clock display.
- **Attendance Statistics Bug Fix (August 2025)**: Fixed critical multi-tenancy bug where Today's Total attendance stats were showing data from ALL churches instead of church-specific data. The getAttendanceStats function was missing church_id filtering, causing stat cards to display incorrect aggregated attendance numbers across all churches in the system. Added proper church_id filtering and ensureChurchContext middleware to ensure accurate, church-specific attendance statistics. This resolves the discrepancy between event-specific stats (4 attendees) and general stats (previously showing 6 due to cross-church aggregation).
