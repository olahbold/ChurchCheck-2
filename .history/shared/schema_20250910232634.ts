import { sql, relations } from "drizzle-orm";
  ...
  phone: z.string().optional().or(z.literal("")).transform((v) => {
    if (!v) return null;
    return v.replace(/[^\d+]/g, "");
  }),

  whatsappNumber: z.string().optional().or(z.literal("")).transform((v) => {
    if (!v) return null;
    return v.replace(/[^\d+]/g, "");
  }),

  dateOfBirth: z.string().optional().or(z.literal("")).transform((val) => {
