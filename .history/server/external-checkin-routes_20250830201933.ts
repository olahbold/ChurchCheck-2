...
// POST /api/external-checkin/checkin/:eventUrl
...
router.get('/events/:eventId/external-checkin',
  ...
  async (req, res) => {
    try {
      ...
      if (!req.churchId) return res.status(401).json({ error: 'Church context missing' });
      ...
      const host = req.get('host');
      const fullUrl = eventRow.externalCheckInUrl ? `${proto}://${host}/external-checkin/${eventRow.externalCheckInUrl}` : null;
      ...
      const msg = (err?.cause?.message || err?.message || '').toString();
      if (/certificate|ssl|handshake/i.test(msg)) {
        return res.status(503).json({
          error: 'Database SSL error',
          detail: msg,
          hint: 'Enable SSL in server/db.ts (see ssl option) or add ?sslmode=require to DATABASE_URL',
        });
      }
      return res.status(500).json({ error: 'Failed to get external check-in details', detail: msg });
    }
  }
);
