...
app.post("/api/super-admin/login", async (req, res) => {
  try {
    ...
    // 1) If no super admin exists, check env defaults
    const anyExists = await churchStorage.anySuperAdminExists?.();
    if (anyExists === false) {
      if (email !== DEFAULT_SUPER_ADMIN_EMAIL || password !== DEFAULT_SUPER_ADMIN_PASSWORD) {
        return res.status(401).json({ error: "Invalid credentials" });
      }

      // hash the ENV default password and store
      const passwordHash = await bcrypt.hash(DEFAULT_SUPER_ADMIN_PASSWORD, 12);
      const superAdmin = await churchStorage.createSuperAdmin({
        email: DEFAULT_SUPER_ADMIN_EMAIL,
        passwordHash,
        firstName: DEFAULT_SUPER_ADMIN_FIRST,
        lastName: DEFAULT_SUPER_ADMIN_LAST,
        role: "super_admin",
        isActive: true,
      });
    ...
    if (!superAdmin.passwordHash) {
      console.error("Super admin login error: missing passwordHash for", superAdmin.id);
