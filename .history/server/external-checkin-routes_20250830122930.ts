...
router.post('/events/:eventId/external-check-in/toggle', authenticateToken, ensureChurchContext, requireRole(['admin']), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const fullUrl = updated.externalCheckInUrl
        ? `${protocol}://${host}/external-check-in/${updated.externalCheckInUrl}`
        : null;
...
router.get('/events/:eventId/external-check-in', authenticateToken, ensureChurchContext, async (req: AuthenticatedRequest, res) => {
  try {
    const { eventId } = req.params;
