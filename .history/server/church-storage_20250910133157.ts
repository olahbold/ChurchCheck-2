...
async getSuperAdminByEmail(email: string): Promise<SuperAdmin | null> {
  ...
  async deleteChurchData(churchId: string): Promise<void> {
    ...
    return this.factoryResetTenant(churchId);

  }

  // Add near bottom of ChurchStorage
async clearTenantData(churchId: string): Promise<void> {
  await db.transaction(async (tx) => {
    // Delete in dependency-safe order (add any other per-tenant tables you have)
    ...
    // If you track other tenant-scoped tables, also clear them here:
    // await tx.delete(events).where(eq(events.churchId, churchId));
    // await tx.delete(reportRuns).where(eq(reportRuns.churchId, churchId));
    // await tx.delete(reportConfigs).where(eq(reportConfigs.churchId, churchId));
    // await tx.delete(communicationProviders).where(eq(communicationProviders.churchId, churchId));
    // await tx.delete(messageDeliveries).where(eq(messageDeliveries.churchId, churchId));

    // IMPORTANT: DO NOT delete churchUsers (admins) and NEVER touch superAdmins here.
    // This function is the one your "Clear All Data" button should call.
  });
  
}
async factoryResetTenant(churchId: string): Promise<void> {
  await db.transaction(async (tx) => {
    // First clear tenant data (members/attendance/etc.)
    await this.clearTenantData(churchId);

    // Then remove church-scoped admins & subscription & church itself
    ...
    // NEVER delete from superAdmins here.
  });
}
