...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/churches/kiosk-settings", authenticateToken, ensureChurchContext, async (req: AuthenticatedRequest, res) => {
    try {
  ...
  app.patch("/api/churches/kiosk-settings", authenticateToken, requireRole(['admin']), ensureChurchContext, async (req: AuthenticatedRequest, res) => {
    try {
      const kioskSettingsValidation = z.object({
        kioskModeEnabled: z.boolean(),
        kioskSessionTimeout: z.number().min(5).max(1440), // 5 minutes to 24 hours
      });

      const settings = kioskSettingsValidation.parse(req.body);
      const church = await churchStorage.updateChurch(req.churchId!, settings);
...
Generated by ChurchConnect Super Admin Dashboard`;
  ...
  // Church routes for multi-tenant functionality (disabled to avoid conflicts)
